...
class Conda(HostedResource):
    ...
    def setup(self):
        if subprocess.run(["conda --version"], shell=True, capture_output=False).returncode != 0:
            ...
            subprocess.run(
                f"find {self.install_path} -follow -type f -name '*.a' -delete && find {self.install_path} -follow -type f -name '*.js.map' -delete && {self.install_path}/bin/conda clean -afy", shell=True)
        else:
